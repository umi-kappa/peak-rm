...
**完了済みセットの実績回数編集**: 履歴タイムラインに表示される各完了セットは、カードのタップでセット編集モーダルを開いて実績回数を調整できる。「セット中はカウントを間違えた」ケースをインターバル中に訂正できるようにするため。詳細は設計方針「セット編集モーダル」「実績値の編集ポリシー」参照
...
- **実績回数の編集**: 各セットの ✎ アイコンのタップでセット編集モーダルを開き、実績回数を調整できる（履歴一覧から開いた履歴詳細表示中は読み専用）。詳細は設計方針「セット編集モーダル」「実績値の編集ポリシー」参照
