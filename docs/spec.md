...
- 各ボタンには以下を表示:
  - 種目名
  - **前回セッションの記録**: 履歴一覧と同じフォーマット（例: `82.5 × 8/8/7`）。同一種目で直前のセッション（ステータス問わず）を参照
  - **現状の推定 1RM**: 同じ「直前セッション」の推定 1RM 値（83.· 1RM グラフ」節の計算式を用いる）を `kg` 単位で表示
  - 直前セッションがない場合（初回 / データクリア直後）は「記録なし」と表示
- 「履歴」への導線
...
- 日付ごとに 1 点。同日に複数セッションがある場合は次の優先順位で 1 件を採用する:
  1. `executed` セッションがあれば、その中で最新のもの
  2. `executed` が 1 件も無ければ `aborted` の中で最新のもの
