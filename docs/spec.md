...
#### セッションフローからの離脱（ブラウザ / OS の「戻る」）

- セッションフロー内の遷移は replace で履歴を畳む（「画面遷移」節参照）ため、トレーニング中・インターバル中にブラウザ / OS の「戻る」を押すとホームへ戻る
- 離脱した時点で実行中セッションは終了扱いとし、そこまでの記録はタブクローズ時と同様、中断として履歴に残る。1 セットも完了していなければ何も記録されない（自動再開しない。ブラウザ操作のため確認ダイアログは挟まない）
