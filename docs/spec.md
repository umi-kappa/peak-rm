...
- **Export**: 設定画面の「Export」ボタンで、`sessions` の全データを 1 つの JSON ファイルとしてダウンロード
  ...
  - 書き出しの完了は件数とファイル名を、Import と同じく閉じる操作だけを持つモーダルで表示する。書き出し中（ファイル生成からダウンロード起動まで）の再押下は無視し、同じ内容のファイルを複数生成しない
- **Import**: 設定画面の「Import」ボタンで JSON ファイルを選択
  ...
  - 検証エラーの内容と置換完了（読み込んだ件数）は、いずれも閉じる操作だけを持つモーダルで表示する（画面内に結果テキストを残さず、トーストも持たない）
