...
- 各ボタンには以下を表示:
  ...
  - 直前セッションの `results` が空の場合（セッション開始直後にタブクローズ等で離脱し、空の `aborted` セッションが残ったケース）も記録は `NO LOG` と表示。また全セット実績 0 回など 1RM の計算対象セット（実績 1 回以上）が無い場合、推定 1RM は `—` と表示
...
- 離脱した時点で実行中セッションは終了扱いとし、そこまでの記録はタブクローズ時と同様 `aborted` のまま履歴に残る（自動再開しない。ブラウザ操作のため確認ダイアログは挟まない）
