...
- 各ボタンには以下を表示:
  ...
  - 全セット実績 0 回など 1RM の計算対象セット（実績 1 回以上）が無い場合、推定 1RM は `—` と表示
- 「履歴」への導線
...
- その日の推定 1RM（後述の種目別式で算出した最大値。対象セットが無く 0 の場合は数値を出さず `—` 表示）。実績回数をこの画面で編集した場合は再計算して反映させる
- **実績回数の編集**: 各完了セットのカード（右端に ✎ の目印）のタップでセット編集モーダルを開き、実績回数を調整できる（履歴一覧から開いた履歴詳細表示中は読み専用）。詳細は設計方針「セット編集モーダル」「実績値の編集ポリシー」参照
- 前回トレーニング時（同種目で当該セッションより前の直近の **完遂** セッション）の推定 1RM からの差分。前回データがない、または当日の推定 1RM が 0（全セットスキップ等）なら表示しない（未完遂のセッションは比較対象外）
...
- 推定 1RM を算出できない日（その日の代表セッションが全セットスキップ）は点にしない。0 kg として打つと実在しない谷が描かれるため
...
- **不変条件**: いずれも `useSession` の書き込みが保証する（アプリ内の操作では破れない）
