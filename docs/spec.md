...
- 各ボタンには以下を表示:
  ...
  - **現状の推定 1RM**: 同じ「直前セッション」の推定 1RM 値（「1RM グラフ」節の計算式を用いる）を `kg` 単位で表示。1RM グラフ・履歴は「同日ベスト」を採用する（「1RM グラフ」節の集約規則）ため基準が異なり、同じ日でもホームの数値とグラフの最新点が一致しないことがある（ホームは直近値・グラフはベスト記録という意図的な差）
  ...
  - 単位・ラベルの大文字 / 小文字（`KG` / `kg` など）はデザイン正本（`docs/design/source/screens-min.jsx`）の表記に従う
- 「履歴」への導線
...
- **`executed` の定義**: 全 `menu.sets` セットの記録があり（`results.length === menu.sets`）、かつ全セットで `actualReps ≥ menu.reps`（= 目標回数）を満たすとき、ステータスは `executed` となる（データモデル節の `isExecuted` 参照）。中断などで `results` が `menu.sets` に満たないセッションは、記録済みセットの内訳によらず常に `aborted`。すなわち `executed` は常に linear progression のトリガー条件を満たし、`aborted` セッションは増量トリガーの対象外
...
- セッションフロー内（メニュー設定 → トレーニング中 → インターバル中 → 結果確認）の画面遷移は、すべてブラウザ履歴を置き換える形（replace）で行い、履歴を常に「ホーム → 現在画面」に畳む
- そのためトレーニング中・インターバル中にブラウザ / OS の「戻る」を押すとホームへ戻る。離脱した時点で実行中セッションは終了扱いとし、そこまでの記録はタブクローズ時と同様 `aborted` のまま履歴に残る（自動再開しない。ブラウザ操作のため確認ダイアログは挟まない）
