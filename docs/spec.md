...
- 実績のみを記録する。**セット単位の「実績」**（実績回数、ただし 0 回は「実績上スキップ」と同義）と、**セッション単位の「ステータス」**（実行 `executed` / 中断 `aborted`）は層が異なる概念として扱う
- 1RM の向上を目的とする
...
セッション中にタブを閉じる / リロードした場合、そこまでに完了したセットは履歴に `aborted` として残る（増分保存のため）。1 セットも完了していなければ何も記録されない。次回起動時はホームから新しいセッションを開始する（自動再開は行わない）。
...
- **`executed` の定義**: 全 `menu.sets` セットの記録があり、かつ全セットで `actualReps ≥ menu.reps`（= 目標回数）を満たすとき、ステータスは `executed` となる（データモデル節の `isExecuted` 参照）。中断などで `results` が `menu.sets` に満たないセッションは、記録済みセットの内訳によらず常に `aborted`。すなわち `executed` は常に linear progression のトリガー条件を満たし、`aborted` セッションは増量トリガーの対象外
...
- セッションのステータス: `isExecuted` 判定に従う（全セット目標達成なら `executed`、目標未達のセットがあれば `aborted` のまま確定）
...
- 離脱した時点で実行中セッションは終了扱いとし、そこまでの記録はタブクローズ時と同様 `aborted` のまま履歴に残る。1 セットも完了していなければ何も記録されない（自動再開しない。ブラウザ操作のため確認ダイアログは挟まない）
...
- 種目名（ヘッダー）/ **ステータスマーカー（3 状態）**: `SESSION ABORTED`（未実施セットあり = 中断）/ `SESSION EXECUTED`（全セット完走したが目標未達）/ `SESSION COMPLETE`（完遂 = `isExecuted`。チェックアイコン + アクセント表示）。`Session.status` の 2 値とは別の表示軸で、`results` の実態から導出する
...
- 前回トレーニング時（同種目で当該セッションより前の直近 **`executed`** セッション）の推定 1RM からの差分。前回データがない、または当日の推定 1RM が 0（全セットスキップ等）なら表示しない（`aborted` セッションは比較対象外）
...
// executed の判定: 全セットの記録があり、かつ全セットで目標回数以上。
// results が menu.sets に満たない（= 中断）場合は常に false
function isExecuted(session: Session): boolean {
  return (
...
- **永続化のタイミング**: 最初のセット完了時に `status = 'aborted'`（保守的なデフォルト）・実績 1 件込みで 1 回 insert する（セッション開始時には保存しない）。以降のセット完了ごとに `results` を増分 update。最終セット完了時に `status = 'executed'` へ更新（`menu.sets = 1` の場合は insert 時点で確定）、中断ボタン時はそのまま `aborted` で確定。タブクローズ / クラッシュ / SW autoUpdate 経由のリロードでも、それまでの実績は `aborted` セッションとして履歴に残る
