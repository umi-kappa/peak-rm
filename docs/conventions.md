...
- **ブラウザ API の glue（`composables/shared/platform/`）も同じ配線に乗せる**。`useAudioCue`（AudioContext）と `useWakeLock`（WakeLockSentinel）はセッションフロー全体で状態を保持する必要がある一方、インターバル画面はセットごとに `router.replace` で再マウントされるため画面内では保持できない。`main.ts` が単一インスタンスを生成して `app.provide()` し、メニュー / トレーニング / インターバルが `inject` で受ける（stories は `src/stories/platform.ts` の fake を provide する）。解除は画面のライフサイクルではなく実行中セッションの終端に紐づけ、その配線（`installSessionEndRelease`）は `installErrorBoundary` と同じく単体テストできるモジュールへ出す。背景化で自動解除された Wake Lock を前景復帰で取り直す配線（`installSessionKeepAwake`）も同じ形で `main.ts` に置き、`document` を引数で差し替えて単体テストする。挙動は spec「Wake Lock のライフサイクル」を参照する
- **セッションフロー離脱の確認は router のガードが担い、ダイアログは `App.vue` が描く**。トレーニング中・インターバル中からフロー外へ出る遷移（ブラウザ / OS の戻る・AppBar の ←）は `router/index.ts` の `beforeEach` が `useSessionLeaveConfirm` の `request()` で問い合わせ、キャンセルならセット完了の書き込みが終わるのを `session.settled()` で待ってから、phase が元の画面のままなら `false` で遷移を取り消し、進んでいれば phase の画面への redirect を返す。`App.vue` は同じインスタンスの `pending` を見て `ConfirmDialog` を描き、`confirm` / `cancel` で問い合わせを解決する。**ダイアログが関わった遷移の行き先はガードだけが決める。永続化の待ちを挟むページは、待っている間に離脱確認が開いたら（答えの前後を問わず）フロー内の遷移を発行しない**（現状はトレーニング画面の `completeSet` のみ。`generation` を await の前後で比べて `replace` を控える）。戻るで履歴の位置はホームへ移っており、そこで `replace` するとホームの履歴を上書きするため。戻る由来のガードの redirect は push なので「ホーム → 現在画面」に収まる。待ちを挟まないページ（インターバル画面の `nextSet` / 中断）は、ダイアログが開いている間そもそも操作を受け付けないので対象外。ページの `onBeforeRouteLeave` に置かないのは、Storybook では `RouterView` の外で描画されるためガードが無効になり検証できないから。挙動は spec「セッションフローからの離脱」を参照する
...
- **ダイアログの初期フォーカスはネイティブ（`showModal()`）に任せ、置き場所は中身が決める**。`BaseDialog` は `autofocus` を持たず、既定では中身の最初のフォーカス可能要素に当たる。**その要素を押すと値が変わる・破棄されるダイアログでは、中身の側で安全な要素に `autofocus` を置く**（`SetEditDialog` は SAVE。既定のままだと Decrease に当たり、開いた直後の Enter がそのまま減算になる）。`ConfirmDialog` は先頭のキャンセルに当たるのが安全側なので `autofocus` を置かない。入力欄には置かない（モバイルで開くたびキーボードが出る）
...
- **例外: 画面内部の状態でしか到達できない見た目（確認ダイアログ・結果モーダルなど）を snapshot に残す場合に限り、その状態へ到達させるためだけの `play` を持つ visual story を許容する**（`disableSnapshot` は付けない）。この `play` には `expect` による振る舞いの検証を置かず、状態への到達で終える（振る舞いは別立ての `Behavior` が持つ）。表示条件が prop / args にあるならこの例外には当たらないので args で到達させる（例外に当たるのは `pages/settings` の `ImportFailed` / `ImportConfirm` / `ExportDone`）
...
- **ボタン共通の interaction reset**（`font: inherit`・`cursor: pointer`・`-webkit-tap-highlight-color: transparent`・長押しでの選択ハンドル / コールアウトの抑止（`user-select: none`・`-webkit-touch-callout: none`））は `global.css` の `:where(button, a)` ルールに置く。レシピ（見た目）と違い全ボタン無条件のリセットなので、各コンポーネントで繰り返さない。`router-link` 版で `<a>` になるボタン（`CardButton` / `IconButton` / `NavLink`）も同じルールで受けるため、セレクタに `a` を含める（`:where()` で詳細度を 0 にし、各コンポーネントの指定が必ず勝つようにする）。`<a>` 固有の `text-decoration: none` だけはリセットではなく各コンポーネント側に置く。ユーザーが書いたテキストを内包するボタン（`TimelineSetCard` の完了セット）は、その本文にだけ `user-select: text` と `-webkit-touch-callout: default` を戻す。**`ul` の余白のリセット**（`margin` / `padding`）も同じ扱いで `global.css` に置く（一覧はすべて読み上げのための list で、箇条書きとしては使わない）。マーカーのリセットは `ul[role='list']` に絞り、`role` を書き忘れた `ul` にマーカーが残るようにする（セマンティクスの欠落を見た目に出す）
...
- **`disabled` の見た目は色を一段落とす（`--color-text-tertiary`）+ `cursor: default`**。あわせて hover / active を `&:not(:disabled)` で囲い、押せない間は反応しないようにする（`IconButton`）。**`BaseButton` は variant を問わず面を `--color-bg-light`・枠線を `--color-line` に置く**（primary はアクセントの塗りが外れて主張が止まり、secondary は透明だった面にカードの塗りが付く。disabled の表現を variant で分岐させない）。どの状態に `disabled` を使うかの判断基準は「状態表現（操作できない状態）」節を参照する
