 ...
 * - 0 秒到達では止まらない。止まるのは超過上限（+3:00 到達）とスコープ破棄の 2 つだけ。
 *   「0 秒に到達していて、まだ止まっていない」区間は reachedZero が表す
...
export function useIntervalTimer(deps: IntervalTimerDeps = {}) {
  ...
  const nowMs = ref(0)
  // カウント中かどうか。0 秒到達では落ちず、超過上限到達とスコープ破棄でのみ false になる
  ...
  // 0 秒に到達し、まだカウント中である区間。start 前（remainingMs も 0）は false、
  // 超過上限で凍結すると false に戻る。インターバル画面はこれを見て通知音を鳴らす区間を決める
  const reachedZero = computed(() => running.value && remainingMs.value === 0)
